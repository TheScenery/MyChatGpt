...
export function activate(context: vscode.ExtensionContext) {
    ...
    let disposable = vscode.commands.registerCommand('extension.myChatGPT', () => {
        ...
        Promise.all([openAIKey, chatPrompt]).then((values) => {
            ...
            if (!key || !prompt) {
                vscode.window.showErrorMessage('Please provide an API key and a chat prompt');
                return;
            }

            const socket = io('https://socket.openai.com', {
                path: '/socket.io',
            ...
            socket.on('model-list', (models: { id: string; objectName: string }[]) => {
                if (!models || !models.length) {
                    vscode.window.showErrorMessage('Could not retrieve available models');
                    return;
                }
